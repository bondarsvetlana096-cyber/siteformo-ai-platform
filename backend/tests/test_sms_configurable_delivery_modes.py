...
def exercise(mode, outcomes, **changes):
    ...
    result = asyncio.run(service.send(
        example_id="TRUSTED_EXAMPLE", phone="+12025550124", message="Please call me tomorrow.",
...
def test_public_contract_exposes_no_routing_authority() -> None:
    ...
    with pytest.raises(Exception):
        SmsDemoRequest(
            first_name="Oleh", phone="+12025550124", message="Hello",
...
@pytest.mark.parametrize(
    ("phone", "message", "error"),
    [(None, "Valid message", "invalid_e164_phone"), ("+12025550124", "", "sms_message_required")],
)
def test_visitor_required_fields_fail_before_provider(phone, message, error) -> None:
    ...
    with pytest.raises(SmsDeliveryError, match=error):
        asyncio.run(service.send(
            example_id="TRUSTED_EXAMPLE", phone=phone, message=message, first_name=None,
...
def test_both_dry_run_has_two_hash_only_audit_candidates_and_zero_calls() -> None:
    result = build_dry_run(
        ...
        message="Please call me tomorrow.", idempotency_key="sms-config-mode-key-0001",
