...
PostgreSQL is canonical storage in `assistant_visitors`, `assistant_conversations`, and `assistant_messages`. Assistant models have separate metadata and are never part of startup `create_all()`. Alembic revision `0007_assistant_core_v1`, following `0006_design_screenshot_flow`, owns the schema.
