...
class Order(Base):
    ...
    deposit_status: Mapped[str] = mapped_column(String(32), default="not_started", nullable=False)
    deposit_paid_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True))
    deposit_amount_cents: Mapped[int | None] = mapped_column(Integer)
    deposit_currency: Mapped[str | None] = mapped_column(String(3))
    brief_confirmed_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True))
    legal_terms_version: Mapped[str | None] = mapped_column(String(128))
    legal_confirmed_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True))
    prepayment_summary_email_status: Mapped[str] = mapped_column(String(16), default="pending", nullable=False)
    prepayment_summary_email_sent_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True))
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now(), nullable=False)
    updated_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now(), onupdate=func.now(), nullable=False)
