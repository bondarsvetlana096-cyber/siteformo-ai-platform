...
class RuntimeBindingService:
    ...
    async def create(
        ...
        return BindingCreationResult(
            await self.deep_links.create(
                origin=origin, validated_name=validated_name,
...
class UnifiedTelegramIngress:
    """Single authenticated, fail-closed Telegram Example ingress."""
    ...
    async def handle(
        ...
        if not self.is_binding_update(payload):
