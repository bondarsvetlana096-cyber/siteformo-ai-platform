...
async def send_email(to: str, subject: str, html: str):
    ...
    from_email = os.getenv("EMAIL_FROM") or "SiteFormo <onboarding@resend.dev>"
    owner_email = os.getenv("OWNER_EMAIL") or getattr(settings, "owner_email", None)
...
class OwnerEmailComposer:
    ...
    def _owner_email() -> str:
        return os.getenv("OWNER_EMAIL") or getattr(settings, "owner_email", "klon97048@gmail.com")
    ...
    def compose_order_email(order) -> dict:
        ...
        description = escape(getattr(order, "desired_site_description", "") or "Not provided")
        ...
        reasoning = escape(getattr(order, "pricing_reasoning", "") or "Not provided")
        ...
        <p style="margin-top:24px;">
            <a href="{approve_url}" style="background:#16a34a;color:white;padding:12px 18px;text-decoration:none;border-radius:8px;">
                APPROVE PAYMENT
            </a>
            &nbsp;
            <a href="{reject_url}" style="background:#dc2626;color:white;padding:12px 18px;text-decoration:none;border-radius:8px;">
                REJECT
            </a>
        </p>
        """
